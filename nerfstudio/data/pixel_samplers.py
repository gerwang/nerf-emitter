...
class PixelSamplerConfig(InstantiateConfig):
    ...
    masked_sampling: bool = True
    """Whether it uses masks when sampling"""
...
class PixelSampler:
    ...
    def __init__(self, config: PixelSamplerConfig, **kwargs) -> None:
        ...
        self.config.masked_sampling = self.kwargs.get("masked_sampling", self.config.masked_sampling)
    ...
    def sample_method(
        ...
        """
        if self.config.masked_sampling and isinstance(mask, torch.Tensor):
    ...
    def collate_image_dataset_batch(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        if self.config.masked_sampling and "mask" in batch:
...
class PatchPixelSampler(PixelSampler):
    ...
    def sample_method(
        ...
        if self.config.masked_sampling and isinstance(mask, Tensor):
