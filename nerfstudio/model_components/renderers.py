...
class RGBRenderer(nn.Module):
    ...
    def __init__(self, background_color: BackgroundColor = "random", hdr=False) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if not self.training:
            max_v = None if self.hdr else 1.0
            torch.clamp_(rgb, min=0.0, max=max_v)
        return rgb
...
class DepthRenderer(nn.Module):
    ...
    Depth Method:
        ...
        - contrib: Maximum contribution along ray.

    Args:
        method: Depth calculation method.
    """

    def __init__(self, method: Literal["median", "expected", "contrib"] = "median") -> None:
        super().__init__()
        self.method = method
    ...
    def forward(
        ...
        ray_indices: Optional[Int[Tensor, "num_samples"]] = None,
        num_rays: Optional[int] = None,
        values: Optional[Float[Tensor, "*batch num_samples 1"]] = None,
        ...
        Args:
            ...
            ray_indices: Ray index for each sample, used when samples are packed.
            num_rays: Number of rays, used when samples are packed.
            values: Values that weights the contribution
        ...
        if self.method == "expected":
            ...
            return depth
        if self.method == "contrib":
            steps = (ray_samples.frustums.starts + ray_samples.frustums.ends) / 2

            if ray_indices is not None and num_rays is not None:
                raise NotImplementedError("Median depth calculation is not implemented for packed samples.")
            contrib = torch.sum(weights * values, dim=-1)
            max_index = torch.max(contrib, dim=-1, keepdim=True).indices
            max_depth = torch.gather(steps[..., 0], dim=-1, index=max_index)
            return max_depth
