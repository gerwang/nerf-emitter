...
def list_images(data: Path) -> List[Path]:
    ...
    """
    allowed_exts = [".jpg", ".jpeg", ".png", ".tif", ".tiff"] + ALLOWED_RAW_EXTS + ALLOWED_HDR_EXTS
...
def resize_image(image_dir, downscale_dirs, copied_image_path, start_downscale):
    img = cv2.imread(f'{image_dir / copied_image_path.name}', cv2.IMREAD_UNCHANGED)
    height, width = img.shape[:2]
    for i in range(start_downscale, len(downscale_dirs)):
        down_img = cv2.resize(img, (width // (2 ** i), height // (2 ** i)), interpolation=cv2.INTER_AREA)
        cv2.imwrite(f'{downscale_dirs[i] / copied_image_path.name}', down_img)
...
def copy_images_list(
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        if keep_image_name:
            copied_image_path = image_dir / f'{image_prefix}{image_path.name}'
        else:
        ...
        if not skip_copy:
            try:
                ...
                if image_path.suffix.lower() in ALLOWED_RAW_EXTS:
                    if keep_image_name:
                        copied_image_path = image_dir / f'{image_path.stem}{RAW_CONVERTED_SUFFIX}'
                    else:
                    ...
                    if use_symlink:
                        os.symlink(os.path.relpath(image_path, os.path.dirname(copied_image_path)), copied_image_path)
                    else:
                    ...
                    run_command(ffmpeg_cmd, verbose=verbose)
                if image_path.suffix.lower() in ALLOWED_HDR_EXTS:
                    has_hdr = True
    ...
    for dir in downscale_dirs:
        dir.mkdir(parents=True, exist_ok=True)
    if has_hdr or keep_image_name:
        num_threads = 20
        num_threads = min(num_threads, multiprocessing.cpu_count() - 1)
        num_threads = max(num_threads, 1)
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            all_tasks = [executor.submit(resize_image, image_dir, downscale_dirs, copied_image_path, start_downscale)
                         for copied_image_path in copied_image_paths]
            concurrent.futures.wait(all_tasks, return_when=concurrent.futures.ALL_COMPLETED)
    else:
